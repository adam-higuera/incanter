    ...
    (let [opts (when options (apply assoc {} options))
          ...
          _group-by (when (:group-by opts)
                      (if (coll? (:group-by opts))
                        (to-list (:group-by opts))
                        ($ (:group-by opts) data)))
          group-keys (when (:group-by opts)
                        (if (coll? (:group-by opts))
                          (distinct (:group-by opts))
                          (map #((:group-by opts) %) (keys ($group-by (:group-by opts) data)))))
          ...
          series-lab (if (:series-label opts)
                       (if (and group-keys (fn? (:series-label opts)))
                         ((:series-label opts) (first group-keys))
                         (:series-label opts))
                       (if x-groups
                         (format "%s, %s (0)" 'x 'y)
                         (format "%s, %s" 'x 'y)))
           ...
           _ (when x-groups
                (doseq [i (range 1 (count x-groups))]
                  (add-lines chart (nth x-groups i)
                             (nth y-groups i)
                             :series-label (if (and (:series-label opts) (fn? (:series-label opts)))
